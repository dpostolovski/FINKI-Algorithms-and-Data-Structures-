...
public class Paytolls
{
    public static void main(String args[]) throws Exception
    {
        ...
        Integer gradovi[] = new Integer[cities];
        for (int i = 0; i < gradovi.length; i++) {
            gradovi[i] = i;
        }


        Graph<Integer> graph = new Graph<>(cities, gradovi);

        StringTokenizer st = new StringTokenizer(br.readLine(), " ");

        int Skopje = Integer.parseInt(st.nextToken());
        int Berlin = Integer.parseInt(st.nextToken());
        ...
        float[] rastojanija = graph.dijkstra(Skopje-1);
        System.out.println(((int) rastojanija[Berlin - 1]));
    }
}