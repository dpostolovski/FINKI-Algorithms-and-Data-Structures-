...
class Graph<E> {
    ...
    public void pathCount(int nodeIndex,int pathLength){
        if(pathLength ==0)
        {
            pathsWithLenghtN++;
            return;
        ...
        for(int i =0; i < num_nodes; i++)
        {
            if(adjacent(nodeIndex, i) == 1)
            {
                pathCount(i, pathLength -1);
            }
        }
    }
...
public class PathCount {
    ...
    public static void main(String[] args) throws Exception {
        ...
        g.pathCount(nodeIndex, pathLength);

        System.out.println(g.pathsWithLenghtN);
        br.close();
